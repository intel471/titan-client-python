...
CVEREPORTSGET412RESPONSE_ONE_OF_SCHEMAS = ["str"]

class CveReportsGet412Response(BaseModel):
    """
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = CveReportsGet412Response.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in CveReportsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in CveReportsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into CveReportsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into CveReportsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return instance

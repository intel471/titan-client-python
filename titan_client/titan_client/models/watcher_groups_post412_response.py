...
WATCHERGROUPSPOST412RESPONSE_ONE_OF_SCHEMAS = ["str"]

class WatcherGroupsPost412Response(BaseModel):
    """
    WatcherGroupsPost412Response
    """
    # data type: str
    oneof_schema_1_validator: Optional[StrictStr] = Field(default=None, description="EmptyJsonBodyError: Received request with empty JSON body")
    # data type: str
    oneof_schema_2_validator: Optional[StrictStr] = Field(default=None, description="JsonBodyParsingError: JSON body is corrupted")
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = WatcherGroupsPost412Response.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in WatcherGroupsPost412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in WatcherGroupsPost412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into WatcherGroupsPost412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into WatcherGroupsPost412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return instance

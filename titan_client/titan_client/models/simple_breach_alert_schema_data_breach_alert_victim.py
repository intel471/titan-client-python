...
class SimpleBreachAlertSchemaDataBreachAlertVictim(BaseModel):
    """
    Purported victim information.
    """ # noqa: E501
    country: Optional[StrictStr] = Field(default=None, description="Purported victim's `country`.")
    industries: List[SimpleBreachAlertSchemaDataBreachAlertVictimIndustriesInner] = Field(description="Purported victim's `industries`.")
    name: StrictStr = Field(description="Purported victim's `name`.")
    region: StrictStr = Field(description="Purported victim's `region`.")
    revenue: StrictStr = Field(description="Purported victim's `revenue`.")
    urls: List[StrictStr] = Field(description="List of purported victim's `urls`.")
    __properties: ClassVar[List[str]] = ["country", "industries", "name", "region", "revenue", "urls"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "industries": [SimpleBreachAlertSchemaDataBreachAlertVictimIndustriesInner.from_dict(_item) for _item in obj["industries"]] if obj.get("industries") is not None else None,
            "name": obj.get("name"),

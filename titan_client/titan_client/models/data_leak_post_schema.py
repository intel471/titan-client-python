...
class DataLeakPostSchema(BaseModel):
    """
    DataLeakPostSchema
    """ # noqa: E501
    chunk_number: Optional[StrictInt] = Field(default=None, description="Data leak post file listing search terms chunk number in which search term was found. File listing search terms related to specific post are stored as chunks.")
    var_date: StrictInt = Field(description="The `date` of the data leak post.", alias="date")
    file_listing: Optional[DataLeakPostSchemaFileListing] = None
    last_updated: Optional[StrictInt] = Field(default=None, description="Last modification date.")
    links: DataLeakPostSchemaLinks
    message: StrictStr = Field(description="HTML data leak post content.")
    uid: StrictStr = Field(description="Unique data leak post identifier.")
    __properties: ClassVar[List[str]] = ["chunk_number", "date", "file_listing", "last_updated", "links", "message", "uid"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "chunk_number": obj.get("chunk_number"),
            "date": obj.get("date"),
            "file_listing": DataLeakPostSchemaFileListing.from_dict(obj["file_listing"]) if obj.get("file_listing") is not None else None,
            "last_updated": obj.get("last_updated"),
            "links": DataLeakPostSchemaLinks.from_dict(obj["links"]) if obj.get("links") is not None else None,

...
class GirSchemaDataGir(BaseModel):
    """
    Sub-document containing GIR information.
    """ # noqa: E501
    description: Optional[StrictStr] = Field(default=None, description="GIR `description`.")
    name: StrictStr = Field(description="GIR `name`.")
    parent: Optional[Dict[str, Any]] = Field(default=None, description="`GIR` that is `parent` of the current one. Has the same structure as described above and may also contain `parent` `GIR`s. <br />If GIR has a `parent` object, `parent` will contain a `path` and `name` field and optionally description and another `parent` object.")
    path: StrictStr = Field(description="GIR `path`.")
    __properties: ClassVar[List[str]] = ["description", "name", "parent", "path"]

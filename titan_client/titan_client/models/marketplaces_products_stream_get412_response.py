...
MARKETPLACESPRODUCTSSTREAMGET412RESPONSE_ONE_OF_SCHEMAS = ["str"]

class MarketplacesProductsStreamGet412Response(BaseModel):
    """
    ...
    # data type: str
    oneof_schema_10_validator: Optional[StrictStr] = Field(default=None, description="InaccessibleDataRequestError: User has no permission to access requested data")
    # data type: str
    oneof_schema_11_validator: Optional[StrictStr] = Field(default=None, description="MalformedCursorError: Stream cursor is malformed")
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = MarketplacesProductsStreamGet412Response.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in MarketplacesProductsStreamGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in MarketplacesProductsStreamGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into MarketplacesProductsStreamGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into MarketplacesProductsStreamGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return instance

...
class SimpleSpotReportSchemaData(BaseModel):
    """
    Sub-document containing Spot report and linked to in objects.
    """ # noqa: E501
    entities: Optional[List[SimpleSpotReportSchemaDataEntitiesInner]] = Field(default=None, description="List of `entities`.")
    spot_report: SimpleSpotReportSchemaDataSpotReport
    __properties: ClassVar[List[str]] = ["entities", "spot_report"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of spot_report
        if self.spot_report:
            _dict['spot_report'] = self.spot_report.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "entities": [SimpleSpotReportSchemaDataEntitiesInner.from_dict(_item) for _item in obj["entities"]] if obj.get("entities") is not None else None,
            "spot_report": SimpleSpotReportSchemaDataSpotReport.from_dict(obj["spot_report"]) if obj.get("spot_report") is not None else None
        })
        return _obj

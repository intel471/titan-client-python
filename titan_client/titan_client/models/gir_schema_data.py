...
class GirSchemaData(BaseModel):
    """
    Sub-document containing GIR data.
    """ # noqa: E501
    gir: GirSchemaDataGir
    __properties: ClassVar[List[str]] = ["gir"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of gir
        if self.gir:
            _dict['gir'] = self.gir.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "gir": GirSchemaDataGir.from_dict(obj["gir"]) if obj.get("gir") is not None else None
        })
        return _obj

...
class PostSchemaLinks(BaseModel):
    """
    Linked data.
    """ # noqa: E501
    author_actor: Optional[PostSchemaLinksAuthorActor] = Field(default=None, alias="authorActor")
    forum: Optional[PostSchemaLinksForum] = None
    images: Optional[List[ImageSchema]] = Field(default=None, description="Array of images (if present).")
    thread: Optional[PostSchemaLinksThread] = None
    __properties: ClassVar[List[str]] = ["authorActor", "forum", "images", "thread"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "authorActor": PostSchemaLinksAuthorActor.from_dict(obj["authorActor"]) if obj.get("authorActor") is not None else None,
            "forum": PostSchemaLinksForum.from_dict(obj["forum"]) if obj.get("forum") is not None else None,
            "images": [ImageSchema.from_dict(_item) for _item in obj["images"]] if obj.get("images") is not None else None,
            "thread": PostSchemaLinksThread.from_dict(obj["thread"]) if obj.get("thread") is not None else None
        })
        return _obj

...
class SituationReportSchemaDataSituationReport(BaseModel):
    """
    Sub-document containing Situation report information.
    """ # noqa: E501
    entities: Optional[List[SituationReportSchemaDataSituationReportEntitiesInner]] = Field(default=None, description="List of entities. Contains the type and value fields of an `entity` object from the entities endpoint.")
    link: SituationReportSchemaDataSituationReportLink
    related_reports: Optional[List[StrictStr]] = Field(default=None, description="Situation report links to related reports like \"Information Report\" or \"Malware Report\".")
    released_at: StrictInt = Field(description="Situation report released date.")
    ...
    text: StrictStr = Field(description="Situation report text.")
    title: Optional[StrictStr] = Field(default=None, description="Situation report title.")
    victims: Optional[List[SituationReportSchemaDataSituationReportVictimsInner]] = Field(default=None, description="Purported victims list.")
    __properties: ClassVar[List[str]] = ["entities", "link", "related_reports", "released_at", "sensitive_source", "text", "title", "victims"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "entities": [SituationReportSchemaDataSituationReportEntitiesInner.from_dict(_item) for _item in obj["entities"]] if obj.get("entities") is not None else None,
            "link": SituationReportSchemaDataSituationReportLink.from_dict(obj["link"]) if obj.get("link") is not None else None,
            "related_reports": obj.get("related_reports"),
            "released_at": obj.get("released_at"),
            "sensitive_source": obj.get("sensitive_source"),
            "text": obj.get("text"),
            "title": obj.get("title"),
            "victims": [SituationReportSchemaDataSituationReportVictimsInner.from_dict(_item) for _item in obj["victims"]] if obj.get("victims") is not None else None
        })
        return _obj

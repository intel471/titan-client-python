...
class WatcherRequestBodyPost(BaseModel):
    """
    WatcherRequestBodyPost
    """ # noqa: E501
    data_sets: Optional[List[StrictStr]] = Field(default=None, description="Limiting watcher by data sets. Defaults to all accessible data sets if empty. Please pay attention, that `Malware Reports` belongs to `malware` data set and `Vulnerability Reports` belong to cve data set.", alias="dataSets")
    description: Optional[StrictStr] = Field(default=None, description="Watcher description.")
    filter_by_gir_set: Optional[StrictStr] = Field(default=None, description="GIR set filter.", alias="filterByGirSet")
    filters: Optional[List[WatcherRequestBodyFiltersInner]] = Field(default=None, description="Search filters. Can be used with `search` watchers for narrowing results. More information about search filter types and their compatibility with search pattern types is [here](https://titan.intel471.com/api/docs/#api-_footer).")
    free_text_pattern: Optional[StrictStr] = Field(default=None, description="Simplified form of adding search pattern. Search type will be automatically set to `FreeText` and pattern will be filled with a given value.", alias="freeTextPattern")
    girs: Optional[List[StrictStr]] = Field(default=None, description="GIR paths selected by user. Ignored if `filterByGirSet` isn't `custom`.")
    notification_channel: Optional[StrictStr] = Field(default=None, description="Notifications channel. email channel will send `email` notifications either `immediately` or `daily` (frequency has to be specified in another field). `website` channel doesn't send emails and keeps all notifications in the website. Regardless of the field value alerts are always accessible via API.", alias="notificationChannel")
    notification_frequency: Optional[StrictStr] = Field(default=None, description="Notification frequency. Applicable to `email` channel only.", alias="notificationFrequency")
    patterns: Optional[List[WatcherRequestBodyPatternsInner]] = Field(default=None, description="Extended form of adding search patterns to a `search` type watcher. Used to specify search pattern type (handle, IP address, hash, etc.).")
    thread_uid: Optional[StrictStr] = Field(default=None, description="Forum thread identifier. Applicable only for `thread` watcher type.", alias="threadUid")
    thread_type: Optional[StrictStr] = Field(default=None, description="In case watcher `type` is `thread` the `threadType` must be specified.", alias="threadType")
    type: Optional[StrictStr] = Field(default=None, description="Watcher type.<br />`search` type watcher queries search engine with a pattern provided,<br />`thread` watcher monitors given forum thread for a new posts.")
    __properties: ClassVar[List[str]] = ["dataSets", "description", "filterByGirSet", "filters", "freeTextPattern", "girs", "notificationChannel", "notificationFrequency", "patterns", "threadUid", "threadType", "type"]

    @field_validator('data_sets')
    def data_sets_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['reports', 'cve', 'forums', 'messagingServices', 'malware', 'marketplaces']):
                raise ValueError("each list item must be one of ('reports', 'cve', 'forums', 'messagingServices', 'malware', 'marketplaces')")
        return value

    @field_validator('filter_by_gir_set')
    def filter_by_gir_set_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['my_girs', 'company_pirs', 'custom']):
            raise ValueError("must be one of enum values ('my_girs', 'company_pirs', 'custom')")
        return value

    @field_validator('notification_channel')
    def notification_channel_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['website', 'email']):
            raise ValueError("must be one of enum values ('website', 'email')")
        return value

    @field_validator('notification_frequency')
    def notification_frequency_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['immediately', 'daily']):
            raise ValueError("must be one of enum values ('immediately', 'daily')")
        return value

    @field_validator('thread_type')
    def thread_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['forums', 'data_leak_blogs']):
            raise ValueError("must be one of enum values ('forums', 'data_leak_blogs')")
        return value
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['search', 'thread']):
            raise ValueError("must be one of enum values ('search', 'thread')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "filterByGirSet": obj.get("filterByGirSet"),
            "filters": [WatcherRequestBodyFiltersInner.from_dict(_item) for _item in obj["filters"]] if obj.get("filters") is not None else None,
            "freeTextPattern": obj.get("freeTextPattern"),
            "girs": obj.get("girs"),
            "notificationChannel": obj.get("notificationChannel"),
            "notificationFrequency": obj.get("notificationFrequency"),
            "patterns": [WatcherRequestBodyPatternsInner.from_dict(_item) for _item in obj["patterns"]] if obj.get("patterns") is not None else None,
            "threadUid": obj.get("threadUid"),

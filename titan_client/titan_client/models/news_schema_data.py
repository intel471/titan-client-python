...
class NewsSchemaData(BaseModel):
    """
    Sub-document containing News data
    """ # noqa: E501
    attachments: Optional[List[NewsSchemaDataAttachmentsInner]] = Field(default=None, description="News `attachments` list")
    released: Optional[StrictInt] = Field(default=None, description="News released date")
    text: Optional[StrictStr] = Field(default=None, description="Raw text with html tags")
    topic: Optional[StrictStr] = Field(default=None, description="`Topic` of the news")
    type: StrictStr = Field(description="Type of News, for example: `BLOG`, `ANNOUNCEMENT`")
    uid: StrictStr = Field(description="Unique news identifier")
    __properties: ClassVar[List[str]] = ["attachments", "released", "text", "topic", "type", "uid"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['BLOG', 'ANNOUNCEMENT']):
            raise ValueError("must be one of enum values ('BLOG', 'ANNOUNCEMENT')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "attachments": [NewsSchemaDataAttachmentsInner.from_dict(_item) for _item in obj["attachments"]] if obj.get("attachments") is not None else None,

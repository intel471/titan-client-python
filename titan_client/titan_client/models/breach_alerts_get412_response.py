...
BREACHALERTSGET412RESPONSE_ONE_OF_SCHEMAS = ["str"]

class BreachAlertsGet412Response(BaseModel):
    """
    BreachAlertsGet412Response
    """
    # data type: str
    oneof_schema_1_validator: Optional[StrictStr] = Field(default=None, description="InvalidSortRangeError: `sort` parameter must be one of the [earliest, latest]")
    # data type: str
    ...
    # data type: str
    oneof_schema_11_validator: Optional[StrictStr] = Field(default=None, description="NoParamError: `<param>` no parameter is specified")
    # data type: str
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = BreachAlertsGet412Response.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in BreachAlertsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in BreachAlertsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into BreachAlertsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into BreachAlertsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return instance

...
class AlertListSchemaReport(BaseModel):
    """
    `Report` object.
    """ # noqa: E501
    admiralty_code: Annotated[str, Field(strict=True)] = Field(description="Code as described [here](http://en.wikipedia.org/wiki/Admiralty_code). All Fintel reports have admiraltyCode=`A1`.", alias="admiraltyCode")
    date_of_information: StrictInt = Field(description="`Date of information` as Epoch Time.", alias="dateOfInformation")
    motivation: List[StrictStr] = Field(description="`Actor`'s `motivation`. CC for Cyber Crime, CE for Cyber Espionage, HA for Hacktivism.")
    portal_report_url: StrictStr = Field(description="URL to the report on the portal.", alias="portalReportUrl")
    source_characterization: StrictStr = Field(description="`Source characterization`.", alias="sourceCharacterization")
    subject: StrictStr = Field(description="Report's subject.")
    uid: StrictStr = Field(description="Unique report identifier.")
    __properties: ClassVar[List[str]] = ["admiraltyCode", "dateOfInformation", "motivation", "portalReportUrl", "sourceCharacterization", "subject", "uid"]

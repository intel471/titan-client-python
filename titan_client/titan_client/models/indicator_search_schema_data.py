...
class IndicatorSearchSchemaData(BaseModel):
    """
    ...
    """ # noqa: E501
    confidence: StrictStr = Field(description="Indicators with `high` confidence are derived from a primary source and are verified. They can safely be used for blocking. If indicators are from a primary source, but they have not been verified, then the confidence rating is reduced to `medium`. We recommend them for alerting or threat hunting. An example of this is a controller URL extracted from a malware configuration file. The file is verified (high), but the controller URLs inside it’s configuration are not (medium). Indicators that are derived from a third party source via pivoting are marked as `low` confidence unless they have been verified. We don’t publish many low confidence indicators.")
    context: Optional[IndicatorSearchSchemaDataContext] = None
    expiration: StrictInt = Field(description="Indicator should be removed from block lists at this date.")
    indicator_data: IndicatorSearchSchemaDataIndicatorData
    indicator_type: StrictStr = Field(description="Describes the type of indicator: (e.g. file, ipv4, url).")
    intel_requirements: Optional[List[StrictStr]] = Field(default=None, description="List of General Intelligence Requirements matching this indicator.")
    mitre_tactics: Optional[StrictStr] = Field(default=None, description="The MITRE ATT&CK tactic classification.")
    threat: IndicatorSearchSchemaDataThreat
    uid: StrictStr = Field(description="Indicator's UID")
    __properties: ClassVar[List[str]] = ["confidence", "context", "expiration", "indicator_data", "indicator_type", "intel_requirements", "mitre_tactics", "threat", "uid"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of indicator_data
        if self.indicator_data:
            _dict['indicator_data'] = self.indicator_data.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "context": IndicatorSearchSchemaDataContext.from_dict(obj["context"]) if obj.get("context") is not None else None,
            "expiration": obj.get("expiration"),
            "indicator_data": IndicatorSearchSchemaDataIndicatorData.from_dict(obj["indicator_data"]) if obj.get("indicator_data") is not None else None,
            "indicator_type": obj.get("indicator_type"),
            "intel_requirements": obj.get("intel_requirements"),
            "mitre_tactics": obj.get("mitre_tactics"),
            "threat": IndicatorSearchSchemaDataThreat.from_dict(obj["threat"]) if obj.get("threat") is not None else None,
            "uid": obj.get("uid")
        })
        return _obj

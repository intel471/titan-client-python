...
ACTORSGET412RESPONSE_ONE_OF_SCHEMAS = ["str"]

class ActorsGet412Response(BaseModel):
    """
    ...
    # data type: str
    oneof_schema_12_validator: Optional[StrictStr] = Field(default=None, description="NoParamError: `<param>` no parameter is specified")
    # data type: str
    oneof_schema_13_validator: Optional[StrictStr] = Field(default=None, description="SearchTooComplexError: The number of logic operators (`||`, `OR`, `&&`, `AND`, `-`) is exceeded")
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ActorsGet412Response.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ActorsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ActorsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ActorsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ActorsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return instance

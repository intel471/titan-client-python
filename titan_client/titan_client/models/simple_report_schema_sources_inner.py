...
class SimpleReportSchemaSourcesInner(BaseModel):
    """
    SimpleReportSchemaSourcesInner
    """ # noqa: E501
    index: StrictInt = Field(description="Numerical value that the source is denoted by in the text of the report.")
    title: StrictStr = Field(description="Source `title`.")
    type: Optional[StrictStr] = Field(default=None, description="`Type` of source such as Thread Post, Information Report, etc.")
    url: StrictStr = Field(description="`Url` to resource where the report information came from.")
    __properties: ClassVar[List[str]] = ["index", "title", "type", "url"]

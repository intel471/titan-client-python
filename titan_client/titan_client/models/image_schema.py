...
class ImageSchema(BaseModel):
    ...
    ImageSchema
    """ # noqa: E501
    dimension: Optional[StrictStr] = Field(default=None, description="Dimension of the image.")
    hash: StrictStr = Field(description="Hash value of the image.")
    hash_type: StrictStr = Field(description="Type of hash function to produce hash.", alias="hashType")
    image_original: Optional[StrictStr] = Field(default=None, description="Original image download url. Note: It's just an example. The actual path depends on specific API endpoint like posts, instant messages, etc.", alias="imageOriginal")
    image_sanitized: Optional[StrictStr] = Field(default=None, description="Sanitised image download url. Note: It's just an example. The actual path depends on specific API endpoint like posts, instant messages, etc.", alias="imageSanitized")
    ocr: Optional[StrictStr] = Field(default=None, description="Recognized text from image.")
    original_url: StrictStr = Field(description="Image original url.", alias="originalUrl")
    size: StrictStr = Field(description="Size of the image in bytes.")
    status: Optional[StrictStr] = Field(default=None, description="Image status. If image has no status this field will not be displayed.")
    __properties: ClassVar[List[str]] = ["dimension", "hash", "hashType", "imageOriginal", "imageSanitized", "ocr", "originalUrl", "size", "status"]
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(['validated', 'unsafe', 'reported']):
            raise ValueError("must be one of enum values ('validated', 'unsafe', 'reported')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "imageOriginal": obj.get("imageOriginal"),
            "imageSanitized": obj.get("imageSanitized"),
            "ocr": obj.get("ocr"),

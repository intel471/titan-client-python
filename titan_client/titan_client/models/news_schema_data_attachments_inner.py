...
class NewsSchemaDataAttachmentsInner(BaseModel):
    """
    ...
    mime_type: StrictStr = Field(description="Mimetype")
    name: StrictStr = Field(description="Attachment file `name`")
    size: StrictInt = Field(description="Attachment fie `size` in bytes")
    url: StrictStr = Field(description="`Url` to download attachment")
    __properties: ClassVar[List[str]] = ["malicious", "mime_type", "name", "size", "url"]

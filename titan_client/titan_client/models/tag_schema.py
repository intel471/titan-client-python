...
class TagSchema(BaseModel):
    ...
    TagSchema
    """ # noqa: E501
    report_count: Optional[StrictInt] = Field(default=None, description="Count of reports with this tag.", alias="report-count")
    tag: StrictStr = Field(description="List of `Tags`.")
    use_count: StrictInt = Field(description="Total usage count. Right now this number is equal to the `report_count` field as only [Information Reports](#../paths/reports.yaml) are tagged.  In future might include other tagged entries.")
    __properties: ClassVar[List[str]] = ["report-count", "tag", "use_count"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "report-count": obj.get("report-count"),
            "tag": obj.get("tag"),

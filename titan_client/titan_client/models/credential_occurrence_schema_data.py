...
class CredentialOccurrenceSchemaData(BaseModel):
    """
    Credential occurrence with related objects.
    """ # noqa: E501
    accessed_url: Optional[StrictStr] = Field(default=None, description="Accessed URL.")
    credential: Optional[CredentialOccurrenceSchemaDataCredential] = None
    credential_set: Optional[CredentialOccurrenceSchemaDataCredentialSet] = None
    detected_malware: Optional[Malware] = None
    file_path: Optional[StrictStr] = Field(default=None, description="Credential occurrence file path.")
    __properties: ClassVar[List[str]] = ["accessed_url", "credential", "credential_set", "detected_malware", "file_path"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of detected_malware
        if self.detected_malware:
            _dict['detected_malware'] = self.detected_malware.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "credential": CredentialOccurrenceSchemaDataCredential.from_dict(obj["credential"]) if obj.get("credential") is not None else None,
            "credential_set": CredentialOccurrenceSchemaDataCredentialSet.from_dict(obj["credential_set"]) if obj.get("credential_set") is not None else None,
            "detected_malware": Malware.from_dict(obj["detected_malware"]) if obj.get("detected_malware") is not None else None,

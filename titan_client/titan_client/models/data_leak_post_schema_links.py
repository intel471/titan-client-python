...
class DataLeakPostSchemaLinks(BaseModel):
    """
    Linked data.
    """ # noqa: E501
    blog: Optional[DataLeakPostSchemaLinksBlog] = None
    images: Optional[List[ImageSnakeCaseSchema]] = Field(default=None, description="Array of images (if present).")
    thread: Optional[DataLeakPostSchemaLinksThread] = None
    __properties: ClassVar[List[str]] = ["blog", "images", "thread"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "blog": DataLeakPostSchemaLinksBlog.from_dict(obj["blog"]) if obj.get("blog") is not None else None,
            "images": [ImageSnakeCaseSchema.from_dict(_item) for _item in obj["images"]] if obj.get("images") is not None else None,
            "thread": DataLeakPostSchemaLinksThread.from_dict(obj["thread"]) if obj.get("thread") is not None else None
        })
        return _obj

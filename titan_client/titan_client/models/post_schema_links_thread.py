...
class PostSchemaLinksThread(BaseModel):
    """
    Thread in which the post was found.
    """ # noqa: E501
    count: StrictInt = Field(description="`Count` of posts in thread.")
    topic: Optional[StrictStr] = Field(default=None, description="Thread `topic`. If post topic is translated, this parameter contains translated topic, if not — original.")
    topic_original: Optional[StrictStr] = Field(default=None, description="Original thread topic. This parameter is active if post topic is translated.", alias="topicOriginal")
    uid: StrictStr = Field(description="Unique thread identifier.")
    __properties: ClassVar[List[str]] = ["count", "topic", "topicOriginal", "uid"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "count": obj.get("count"),
            "topic": obj.get("topic"),
            "topicOriginal": obj.get("topicOriginal"),
            "uid": obj.get("uid")
        })
        return _obj

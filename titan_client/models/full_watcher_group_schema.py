...
class FullWatcherGroupSchema(BaseModel):
    """
    FullWatcherGroupSchema
    """ # noqa: E501
    description: Optional[StrictStr] = Field(default=None, description="Type of watcher group ownership.")
    muted: StrictBool = Field(description="Watcher's mute status (a watcher group is muted if all of its watchers are muted)")
    name: StrictStr = Field(description="Name of watcher group.")
    owner: StrictStr = Field(description="Watcher group owner's name.")
    type: StrictStr = Field(description="Type of watcher group ownership.")
    uid: StrictStr = Field(description="Unique watcher group identifier.")
    watchers: List[FullWatcherGroupSchemaAllOfWatchers] = Field(description="List of Watchers. Watchers are displayed only for alerts included in results.")
    __properties: ClassVar[List[str]] = ["description", "muted", "name", "owner", "type", "uid", "watchers"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['owned_by_me', 'public_or_shared_with_me', 'subscribed_to', 'my_ungrouped']):
            raise ValueError("must be one of enum values ('owned_by_me', 'public_or_shared_with_me', 'subscribed_to', 'my_ungrouped')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "type": obj.get("type"),
            "uid": obj.get("uid"),
            "watchers": [FullWatcherGroupSchemaAllOfWatchers.from_dict(_item) for _item in obj["watchers"]] if obj.get("watchers") is not None else None
        })
        return _obj

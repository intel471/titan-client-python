...
class IocSchemaLinks(BaseModel):
    """
    Linked data.
    """ # noqa: E501
    actor_total_count: StrictInt = Field(description="Total count of linked actors.", alias="actorTotalCount")
    actors: Optional[List[IocSchemaLinksActorsInner]] = Field(default=None, description="List of `Actors` <br />`Array` of simple `Actor`. Contains only `uid`, `handles` from [Actors]().")
    report_total_count: StrictInt = Field(description="Total count of linked reports.", alias="reportTotalCount")
    reports: Optional[List[IocSchemaLinksReportsInner]] = Field(default=None, description="Linked `reports` Array of simplified version of one of the following: `Information Report`, `Fintel Report`, `Malware Report`, `Spot Report`, `Situation Report`, `Breach Alert`.")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "actors": [IocSchemaLinksActorsInner.from_dict(_item) for _item in obj["actors"]] if obj.get("actors") is not None else None,
            "reportTotalCount": obj.get("reportTotalCount"),
            "reports": [IocSchemaLinksReportsInner.from_dict(_item) for _item in obj["reports"]] if obj.get("reports") is not None else None
        })
        return _obj

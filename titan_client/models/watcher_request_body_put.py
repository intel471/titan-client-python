...
class WatcherRequestBodyPut(BaseModel):
    """
    ...
    muted: Optional[StrictBool] = Field(default=None, description="Watcher's mute status (if a watcher is muted, no alerts are received during its mute period)")
    type: Optional[StrictStr] = Field(default=None, description="Watcher type.<br />Only watchers of type `search` can be edited.")
    __properties: ClassVar[List[str]] = ["dataSets", "description", "filterByGirSet", "filters", "freeTextPattern", "girs", "notificationChannel", "notificationFrequency", "patterns", "threadUid", "muted", "type"]

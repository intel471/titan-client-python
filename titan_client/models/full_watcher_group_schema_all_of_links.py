...
class FullWatcherGroupSchemaAllOfLinks(BaseModel):
    """
    Watcher linked data. Only for thread watchers.
    """ # noqa: E501
    forum: Optional[FullWatcherGroupSchemaAllOfLinksForum] = None
    thread: Optional[FullWatcherGroupSchemaAllOfLinksThread] = None
    __properties: ClassVar[List[str]] = ["forum", "thread"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "forum": FullWatcherGroupSchemaAllOfLinksForum.from_dict(obj["forum"]) if obj.get("forum") is not None else None,
            "thread": FullWatcherGroupSchemaAllOfLinksThread.from_dict(obj["thread"]) if obj.get("thread") is not None else None
        })
        return _obj

...
class MarketplaceProductOtherProductSchema(BaseModel):
    """
    MarketplaceProductOtherProductSchema
    """ # noqa: E501
    availability: Optional[MarketplaceProductAvailabilitySchema] = None
    categories: Optional[List[StrictStr]] = Field(default=None, description="Product categories.")
    ...
    __properties: ClassVar[List[str]] = ["availability", "categories", "description", "marketplace", "product_type", "title"]
    ...
    def product_type_validate_enum(cls, value):
        ...
        if value not in set(['Other product']):
            raise ValueError("must be one of enum values ('Other product')")
        return value

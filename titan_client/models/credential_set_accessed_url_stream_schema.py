...
class CredentialSetAccessedUrlStreamSchema(BaseModel):
    """
    CredentialSetAccessedUrlStreamSchema
    """ # noqa: E501
    activity: CredentialSetAccessedUrlSchemaActivity
    classification: Optional[CredentialSetAccessedUrlSchemaClassification] = None
    data: CredentialSetAccessedUrlStreamSchemaData
    last_updated: StrictInt = Field(description="Credential set accessed url last modification date.")
    uid: StrictStr = Field(description="Unique credential set accessed url identifier.")
    __properties: ClassVar[List[str]] = ["activity", "classification", "data", "last_updated", "uid"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "activity": CredentialSetAccessedUrlSchemaActivity.from_dict(obj["activity"]) if obj.get("activity") is not None else None,
            "classification": CredentialSetAccessedUrlSchemaClassification.from_dict(obj["classification"]) if obj.get("classification") is not None else None,
            "data": CredentialSetAccessedUrlStreamSchemaData.from_dict(obj["data"]) if obj.get("data") is not None else None,

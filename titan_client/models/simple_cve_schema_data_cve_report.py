...
class SimpleCveSchemaDataCveReport(BaseModel):
    """
    Sub-document containing report information.
    """ # noqa: E501
    activity_location: Optional[SimpleCveSchemaDataCveReportActivityLocation] = None
    counter_measure_links: Optional[List[SimpleCveSchemaDataCveReportCounterMeasureLinksInner]] = Field(default=None, description="Titled URLs to countermeasure information to protect against the CVE.")
    counter_measures: Optional[StrictStr] = Field(default=None, description="Summary of `countermeasures` to protect against the CVE.")
    cpe: Optional[Dict[str, Any]] = Field(default=None, description="`CPE` (Common Platform Enumeration) is the list of the software affected by the vulnerability. Raw data field.")
    cve_status: StrictStr = Field(description="`status_new` for recently added CVE; `status_existing` for CVE being reported for a while; `status_historical` for phased out and not actual at the moment. Allowed values: `status_existing`, `status_new`, `status_historical`.")
    cve_type: StrictStr = Field(description="Type of CVE, for example: `Buffer overflow`, `Privilege escalation`, `Memory corruption`, etc.")
    cvss_score: Optional[SimpleCveSchemaDataCveReportCvssScore] = None
    detection: Optional[StrictStr] = Field(default=None, description="Detection (signatures, definitions) exists for the CVE. Allowed values: `available`, `not_available`.")
    exploit_status: Optional[SimpleCveSchemaDataCveReportExploitStatus] = None
    interest_level: Optional[SimpleCveSchemaDataCveReportInterestLevel] = None
    name: StrictStr = Field(description="CVE number.")
    patch_links: Optional[List[SimpleCveSchemaDataCveReportPatchLinksInner]] = Field(default=None, description="Titled URLs to available CVE patch.")
    patch_status: Optional[StrictStr] = Field(default=None, description="Indicates availability of the CVE patch. Allowed values: `available`, `some_available`, `unavailable`.")
    poc: StrictStr = Field(description="Proof of concept code or demonstration exists. Allowed values: `observed`, `not_observed`, `alleged_observed`.")
    poc_links: Optional[List[SimpleCveSchemaDataCveReportPocLinksInner]] = Field(default=None, description="Titled URLs to Proofs of Concept of the CVE.")
    product_name: Optional[StrictStr] = Field(default=None, description="`Product name` of the affected software.")
    risk_level: StrictStr = Field(description="Intel471 `risk level` of the described CVE. Allowed values: `high`, `medium`, `low`.")
    summary: Optional[StrictStr] = Field(default=None, description="Intel471 `summary` of the CVE.")
    titan_links: Optional[List[SimpleCveSchemaDataCveReportTitanLinksInner]] = Field(default=None, description="Links to the related titan items.")
    underground_activity: Optional[StrictStr] = Field(default=None, description="Describes whether `underground activity` is observed for given CVE. Allowed values: `observed`, `not_observed`.")
    underground_activity_summary: Optional[StrictStr] = Field(default=None, description="Describes CVE underground activity.")
    vendor_name: Optional[StrictStr] = Field(default=None, description="`Vendor name` of the affected software.")
    __properties: ClassVar[List[str]] = ["activity_location", "counter_measure_links", "counter_measures", "cpe", "cve_status", "cve_type", "cvss_score", "detection", "exploit_status", "interest_level", "name", "patch_links", "patch_status", "poc", "poc_links", "product_name", "risk_level", "summary", "titan_links", "underground_activity", "underground_activity_summary", "vendor_name"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of activity_location
        if self.activity_location:
            _dict['activity_location'] = self.activity_location.to_dict()
        ...
        if self.counter_measure_links:
            for _item in self.counter_measure_links:
                if _item:
        ...
        # override the default output from pydantic by calling `to_dict()` of exploit_status
        if self.exploit_status:
            _dict['exploit_status'] = self.exploit_status.to_dict()
        # override the default output from pydantic by calling `to_dict()` of interest_level
        if self.interest_level:
            _dict['interest_level'] = self.interest_level.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in patch_links (list)
        _items = []
        if self.patch_links:
            for _item in self.patch_links:
                if _item:
                    _items.append(_item.to_dict())
        ...
        _items = []
        if self.poc_links:
            for _item in self.poc_links:
                if _item:
                    _items.append(_item.to_dict())
            _dict['poc_links'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in titan_links (list)
        _items = []
        if self.titan_links:
            for _item in self.titan_links:
                if _item:
                    _items.append(_item.to_dict())
            _dict['titan_links'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "activity_location": SimpleCveSchemaDataCveReportActivityLocation.from_dict(obj["activity_location"]) if obj.get("activity_location") is not None else None,
            "counter_measure_links": [SimpleCveSchemaDataCveReportCounterMeasureLinksInner.from_dict(_item) for _item in obj["counter_measure_links"]] if obj.get("counter_measure_links") is not None else None,
            "counter_measures": obj.get("counter_measures"),
            "cpe": obj.get("cpe"),
            "cve_status": obj.get("cve_status"),
            "cve_type": obj.get("cve_type"),
            "cvss_score": SimpleCveSchemaDataCveReportCvssScore.from_dict(obj["cvss_score"]) if obj.get("cvss_score") is not None else None,
            "detection": obj.get("detection"),
            "exploit_status": SimpleCveSchemaDataCveReportExploitStatus.from_dict(obj["exploit_status"]) if obj.get("exploit_status") is not None else None,
            "interest_level": SimpleCveSchemaDataCveReportInterestLevel.from_dict(obj["interest_level"]) if obj.get("interest_level") is not None else None,
            "name": obj.get("name"),
            "patch_links": [SimpleCveSchemaDataCveReportPatchLinksInner.from_dict(_item) for _item in obj["patch_links"]] if obj.get("patch_links") is not None else None,
            "patch_status": obj.get("patch_status"),
            "poc": obj.get("poc"),
            "poc_links": [SimpleCveSchemaDataCveReportPocLinksInner.from_dict(_item) for _item in obj["poc_links"]] if obj.get("poc_links") is not None else None,
            "product_name": obj.get("product_name"),
            "risk_level": obj.get("risk_level"),
            "summary": obj.get("summary"),
            "titan_links": [SimpleCveSchemaDataCveReportTitanLinksInner.from_dict(_item) for _item in obj["titan_links"]] if obj.get("titan_links") is not None else None,
            "underground_activity": obj.get("underground_activity"),
            "underground_activity_summary": obj.get("underground_activity_summary"),

...
class SimpleBreachAlertSchemaData(BaseModel):
    """
    Sub-document containing Breach Alert and linked to it `entities`.
    """ # noqa: E501
    breach_alert: SimpleBreachAlertSchemaDataBreachAlert
    entities: Optional[List[SimpleBreachAlertSchemaDataEntitiesInner]] = Field(default=None, description="List of `entities`.")
    __properties: ClassVar[List[str]] = ["breach_alert", "entities"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.breach_alert:
            _dict['breach_alert'] = self.breach_alert.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "breach_alert": SimpleBreachAlertSchemaDataBreachAlert.from_dict(obj["breach_alert"]) if obj.get("breach_alert") is not None else None,
            "entities": [SimpleBreachAlertSchemaDataEntitiesInner.from_dict(_item) for _item in obj["entities"]] if obj.get("entities") is not None else None
        })
        return _obj

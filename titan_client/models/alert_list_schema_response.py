...
class AlertListSchemaResponse(BaseModel):
    """
    AlertListSchemaResponse
    """ # noqa: E501
    alert_total_count: StrictInt = Field(description="Total count of alerts.", alias="alertTotalCount")
    alerts: Optional[List[AlertListSchema]] = Field(default=None, description="List of Alerts.")
    watcher_groups: Optional[List[FullWatcherGroupSchema]] = Field(default=None, description="List of Watcher groups. Groups are displayed only for alerts included in results.", alias="watcherGroups")
    __properties: ClassVar[List[str]] = ["alertTotalCount", "alerts", "watcherGroups"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.alerts:
            ...
            _dict['alerts'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in watcher_groups (list)
        _items = []
        if self.watcher_groups:
            for _item in self.watcher_groups:
                if _item:
                    _items.append(_item.to_dict())
            _dict['watcherGroups'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "alertTotalCount": obj.get("alertTotalCount"),
            "alerts": [AlertListSchema.from_dict(_item) for _item in obj["alerts"]] if obj.get("alerts") is not None else None,
            "watcherGroups": [FullWatcherGroupSchema.from_dict(_item) for _item in obj["watcherGroups"]] if obj.get("watcherGroups") is not None else None
        })
        return _obj

...
class MarketplaceSearchResponse(BaseModel):
    ...
    """ # noqa: E501
    marketplaces: Optional[List[MarketplaceSearchSchema]] = Field(default=None, description="List of marketplaces.")
    marketplaces_count: StrictInt = Field(description="Count of matched results.")
    marketplaces_partial_result: Optional[StrictBool] = Field(default=None, description="Indicates whether response contains partial result. It could be in case when request took too long and was terminated by timeout.")
    __properties: ClassVar[List[str]] = ["marketplaces", "marketplaces_count", "marketplaces_partial_result"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in marketplaces (list)
        _items = []
        if self.marketplaces:
            for _item in self.marketplaces:
                if _item:
                    _items.append(_item.to_dict())
            _dict['marketplaces'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "marketplaces": [MarketplaceSearchSchema.from_dict(_item) for _item in obj["marketplaces"]] if obj.get("marketplaces") is not None else None,
            "marketplaces_count": obj.get("marketplaces_count"),
            "marketplaces_partial_result": obj.get("marketplaces_partial_result")
        })
        return _obj

...
ALERTSGET412RESPONSE_ONE_OF_SCHEMAS = ["str"]

class AlertsGet412Response(BaseModel):
    """
    ...
    # data type: str
    oneof_schema_4_validator: Optional[StrictStr] = Field(default=None, description="InvalidCountRangeError: `count` parameter must be in range [1-100]")
    # data type: str
    oneof_schema_5_validator: Optional[StrictStr] = Field(default=None, description="IllegalOffsetError: `offset` parameter must be an existing alert uid")
    # data type: str
    oneof_schema_6_validator: Optional[StrictStr] = Field(default=None, description="NotANumberError: `<param>` parameter must be a number")
    # data type: str
    oneof_schema_7_validator: Optional[StrictStr] = Field(default=None, description="DuplicatedParamError: `<param>` parameter is duplicated")
    # data type: str
    oneof_schema_8_validator: Optional[StrictStr] = Field(default=None, description="InaccessibleDataRequestError: User has no permission to access requested data")
    # data type: str
    oneof_schema_9_validator: Optional[StrictStr] = Field(default=None, description="InvalidSortRangeError: `sort` parameter must be one of the [earliest, latest]")
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = AlertsGet412Response.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in AlertsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in AlertsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into AlertsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AlertsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return instance

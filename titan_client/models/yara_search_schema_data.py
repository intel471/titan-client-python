...
class YARASearchSchemaData(BaseModel):
    """
    ...
    """ # noqa: E501
    confidence: StrictStr = Field(description="`Confidence` of YARA: `high` — recommended for blocking, `medium` — for alerting, `low` — needs to be verified.")
    intel_requirements: Optional[List[StrictStr]] = Field(default=None, description="List of General Intelligence Requirements matching this YARA.")
    threat: YARASearchSchemaDataThreat
    yara_data: YARASearchSchemaDataYaraData
    __properties: ClassVar[List[str]] = ["confidence", "intel_requirements", "threat", "yara_data"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of yara_data
        if self.yara_data:
            _dict['yara_data'] = self.yara_data.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "threat": YARASearchSchemaDataThreat.from_dict(obj["threat"]) if obj.get("threat") is not None else None,
            "yara_data": YARASearchSchemaDataYaraData.from_dict(obj["yara_data"]) if obj.get("yara_data") is not None else None
        })
        return _obj

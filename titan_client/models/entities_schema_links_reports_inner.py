...
class EntitiesSchemaLinksReportsInner(BaseModel):
    """
    ...
    released: StrictInt = Field(description="Date the `report` was `released.")
    source_characterization: Optional[StrictStr] = Field(default=None, description="Characterization of the reports source.", alias="sourceCharacterization")
    subject: StrictStr = Field(description="Report's `subject`.")
    uid: StrictStr = Field(description="Unique report identifier.")
    __properties: ClassVar[List[str]] = ["admiraltyCode", "dateOfInformation", "motivation", "portalReportUrl", "released", "sourceCharacterization", "subject", "uid"]

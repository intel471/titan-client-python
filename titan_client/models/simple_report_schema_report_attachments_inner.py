...
class SimpleReportSchemaReportAttachmentsInner(BaseModel):
    """
    SimpleReportSchemaReportAttachmentsInner
    """ # noqa: E501
    description: Optional[StrictStr] = Field(default=None, description="A `description` of attachment.")
    file_name: StrictStr = Field(description="Attachment file's name.", alias="fileName")
    file_size: StrictInt = Field(description="Attachment file size in bytes.", alias="fileSize")
    malicious: Optional[StrictBool] = Field(default=None, description="Indicates if attachment is `malicious`. If malicious=`true`, attachment will be an archive with password `infected`.")
    mime_type: StrictStr = Field(description="Mime type.", alias="mimeType")
    url: StrictStr = Field(description="`Url` to download attachment.")
    __properties: ClassVar[List[str]] = ["description", "fileName", "fileSize", "malicious", "mimeType", "url"]

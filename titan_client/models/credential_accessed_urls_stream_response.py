...
class CredentialAccessedUrlsStreamResponse(BaseModel):
    """
    CredentialAccessedUrlsStreamResponse
    """ # noqa: E501
    credential_accessed_urls: Optional[List[CredentialAccessedUrlStreamSchema]] = Field(default=None, description="List of `Credential accessed urls`.")
    credential_accessed_urls_partial_result: Optional[StrictBool] = Field(default=None, description="Indicates whether response contains partial result. It could be in case when request took too long and was terminated by timeout.")
    credential_accessed_urls_total_count: StrictInt = Field(description="Total count of matched credential accessed urls.")
    cursor_next: Optional[StrictStr] = Field(default=None, description="Stream position identifier to continue scrolling from.")
    __properties: ClassVar[List[str]] = ["credential_accessed_urls", "credential_accessed_urls_partial_result", "credential_accessed_urls_total_count", "cursor_next"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in credential_accessed_urls (list)
        _items = []
        if self.credential_accessed_urls:
            for _item in self.credential_accessed_urls:
                if _item:
                    _items.append(_item.to_dict())
            _dict['credential_accessed_urls'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "credential_accessed_urls": [CredentialAccessedUrlStreamSchema.from_dict(_item) for _item in obj["credential_accessed_urls"]] if obj.get("credential_accessed_urls") is not None else None,
            "credential_accessed_urls_partial_result": obj.get("credential_accessed_urls_partial_result"),
            "credential_accessed_urls_total_count": obj.get("credential_accessed_urls_total_count"),

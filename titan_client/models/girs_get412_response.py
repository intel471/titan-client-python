...
GIRSGET412RESPONSE_ONE_OF_SCHEMAS = ["str"]

class GirsGet412Response(BaseModel):
    """
    GirsGet412Response
    """
    # data type: str
    oneof_schema_1_validator: Optional[StrictStr] = Field(default=None, description="InvalidCountRangeError: `count` parameter must be in range [1-100]")
    # data type: str
    oneof_schema_2_validator: Optional[StrictStr] = Field(default=None, description="InvalidOffsetRangeError: `offset` parameter must be in range [0-1000]")
    # data type: str
    oneof_schema_3_validator: Optional[StrictStr] = Field(default=None, description="NotANumberError: `<param>` parameter must be a number")
    # data type: str
    oneof_schema_4_validator: Optional[StrictStr] = Field(default=None, description="DuplicatedParamError: `<param>` parameter is duplicated")
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = GirsGet412Response.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in GirsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in GirsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into GirsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GirsGet412Response with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return instance

...
class DataLeakPostSchemaLinksThread(BaseModel):
    """
    Data leak thread in which the post was found.
    """ # noqa: E501
    count: StrictInt = Field(description="`Count` of posts in thread.")
    topic: Optional[StrictStr] = Field(default=None, description="Data leak thread `topic`. If post topic is translated, this parameter contains translated topic, if not — original.")
    uid: StrictStr = Field(description="Unique data leak thread identifier.")
    __properties: ClassVar[List[str]] = ["count", "topic", "uid"]

...
class PrivateMessageSchemaLinks(BaseModel):
    """
    Linked data.
    """ # noqa: E501
    author_actor: PrivateMessageSchemaLinksAuthorActor = Field(alias="authorActor")
    forum: PrivateMessageSchemaLinksForum
    images: Optional[List[ImageSchema]] = Field(default=None, description="Array of images (if present).")
    recipient_actor: Optional[PrivateMessageSchemaLinksRecipientActor] = Field(default=None, alias="recipientActor")
    thread: PrivateMessageSchemaLinksThread
    __properties: ClassVar[List[str]] = ["authorActor", "forum", "images", "recipientActor", "thread"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of author_actor
        if self.author_actor:
            _dict['authorActor'] = self.author_actor.to_dict()
        ...
        if self.images:
            ...
            _dict['images'] = _items
        # override the default output from pydantic by calling `to_dict()` of recipient_actor
        if self.recipient_actor:
            _dict['recipientActor'] = self.recipient_actor.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "authorActor": PrivateMessageSchemaLinksAuthorActor.from_dict(obj["authorActor"]) if obj.get("authorActor") is not None else None,
            "forum": PrivateMessageSchemaLinksForum.from_dict(obj["forum"]) if obj.get("forum") is not None else None,
            "images": [ImageSchema.from_dict(_item) for _item in obj["images"]] if obj.get("images") is not None else None,
            "recipientActor": PrivateMessageSchemaLinksRecipientActor.from_dict(obj["recipientActor"]) if obj.get("recipientActor") is not None else None,
            "thread": PrivateMessageSchemaLinksThread.from_dict(obj["thread"]) if obj.get("thread") is not None else None
        })
        return _obj

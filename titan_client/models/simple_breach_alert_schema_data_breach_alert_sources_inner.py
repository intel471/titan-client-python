...
class SimpleBreachAlertSchemaDataBreachAlertSourcesInner(BaseModel):
    """
    SimpleBreachAlertSchemaDataBreachAlertSourcesInner
    """ # noqa: E501
    var_date: StrictInt = Field(description="Date of source publication or information acquisition.", alias="date")
    source_type: StrictStr = Field(description="Description of source information's type.")
    title: StrictStr = Field(description="The `title` of the source.")
    type: StrictStr = Field(description="Source `type`, external or internal.")
    url: StrictStr = Field(description="Source link, an absolute `url`.")
    __properties: ClassVar[List[str]] = ["date", "source_type", "title", "type", "url"]

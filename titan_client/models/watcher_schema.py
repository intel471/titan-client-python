...
class WatcherSchema(BaseModel):
    ...
    """ # noqa: E501
    data_sets: Optional[List[StrictStr]] = Field(default=None, description="Data sets the watcher is limited by.", alias="dataSets")
    ...
    filters: Optional[List[WatcherSchemaFiltersInner]] = Field(default=None, description="Search filter of `search` watcher.")
    girs: Optional[List[StrictStr]] = Field(default=None, description="GIR set paths defined by `filterByGirSet` param.")
    links: Optional[List[WatcherSchemaLinksInner]] = Field(default=None, description="Links to the Forum and Thread of the `thread` type watcher.")
    muted: StrictBool = Field(description="Watcher's mute status (if a watcher is muted, no alerts are received during its mute period)")
    notification_channel: StrictStr = Field(description="Notification chanel.", alias="notificationChannel")
    notification_frequency: StrictStr = Field(description="Notification frequency.", alias="notificationFrequency")
    patterns: Optional[List[WatcherSchemaPatternsInner]] = Field(default=None, description="Search query patterns applicable to a watcher of the `search` type.")
    ...
    type: StrictStr = Field(description="Watcher `type`.")
    uid: StrictStr = Field(description="Watcher identifier.")
    __properties: ClassVar[List[str]] = ["dataSets", "description", "filterByGirSet", "filters", "girs", "links", "muted", "notificationChannel", "notificationFrequency", "patterns", "threadType", "type", "uid"]
    ...
    def notification_channel_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['email', 'website']):
            raise ValueError("must be one of enum values ('email', 'website')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "filters": [WatcherSchemaFiltersInner.from_dict(_item) for _item in obj["filters"]] if obj.get("filters") is not None else None,
            "girs": obj.get("girs"),
            "links": [WatcherSchemaLinksInner.from_dict(_item) for _item in obj["links"]] if obj.get("links") is not None else None,
            "muted": obj.get("muted"),
            "notificationChannel": obj.get("notificationChannel"),
            "notificationFrequency": obj.get("notificationFrequency"),
            "patterns": [WatcherSchemaPatternsInner.from_dict(_item) for _item in obj["patterns"]] if obj.get("patterns") is not None else None,
